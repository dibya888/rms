...
async function bootstrap() {
  ...
  app.use((request: Request, response: Response, next: NextFunction) => {
    ...
    if (!existing) response.cookie(csrfCookie, token, { httpOnly: false, secure: process.env.NODE_ENV === 'production', sameSite: 'strict', path: '/' });
    if (['GET', 'HEAD', 'OPTIONS'].includes(request.method) || request.path === '/auth/csrf') return next();
    const provided = request.header('x-csrf-token');
    if (!provided || provided.length !== token.length || !timingSafeEqual(Buffer.from(provided), Buffer.from(token))) return response.status(403).json({ message: 'CSRF token is missing or invalid' });
    return next();
  });
  app.useGlobalPipes(new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true, transform: true }));
  const openApiConfig = new DocumentBuilder().setTitle('RMS API').setDescription('Property and rent management API').setVersion('1.0').addBearerAuth().build();
