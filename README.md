...
5. Deploy the web build with `VITE_API_URL` set to the HTTPS API origin. Restrict `CORS_ORIGIN` to that exact web origin and enable secure cookies. This is a single-page app that reads routes like `/activate`, `/register`, `/forgot-password`, and `/reset-password` from `window.location.pathname` — any static host must rewrite all paths to `index.html` (a 200 rewrite, not a redirect) or those links will 404. The Docker/nginx image already does this via `apps/web/nginx.conf`; deploying to Netlify instead, the included `netlify.toml` and `apps/web/public/_redirects` cover it.
